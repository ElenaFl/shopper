...
const [couponError, setCouponError] = useState("");
  ...
  const handleProceedToCheckout = async () => {
    ...
    try {
      ...
      // ensure user has selected shipping details and updated totals
      if (!selectedCountry || !selectedCity || !selectedPostal || !applied) {
        // If you prefer to allow proceeding without shipping calc, remove the applied check
        alert(
      ...
      // Optional: quick stock validation (uncomment if you want server/local check)
      // const itemsToValidate = cart.map(p => ({ id: p.id, quantity: p.quantity }));
      // const stockRes = await validateServerStock(itemsToValidate);
      // if (!stockRes.ok) {
      //   alert('Unable to validate stock at the moment. Please try again later.');
      //   return;
      // }
      // // optional: inspect stockRes.data and update cart or notify user if shortages

      // Persist current checkout snapshot so the checkout page can read it
      ...
      try {
        // use sessionStorage (cleared when browser/tab closed) — safe for transient checkout state
        sessionStorage.setItem(
          "shopper_checkout",
          JSON.stringify(checkoutSnapshot),
        );
      } catch (err) {
        // sessionStorage might fail in strict privacy modes; fallback to localStorage if desired
        console.warn(
          "Failed to write sessionStorage for checkout snapshot:",
          err,
        );
      }

      // Navigate to internal checkout page
      // If you use react-router (v6+) and have access to navigate:
      // navigate('/checkout'); // ensure you import/use useNavigate from 'react-router-dom'
      // Otherwise fallback to location change:
